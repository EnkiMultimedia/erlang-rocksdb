...
-type write_actions() :: [{put, Key::binary(), Value::iodata()} |
                          {put, ColumnFamilyHandle::cf_handle(), Key::binary(), Value::iodata()} |
                          {delete, Key::binary()} |
...
-type fold_fun() :: fun(({Key::binary(), Value::iodata()}, any()) -> any()).
...
-spec batch_put(Batch :: batch_handle(), Key :: binary(), Value :: iodata()) -> ok.
batch_put(_Batch, _Key, _Value) ->
  ?nif_stub.

%% @doc like `batch_put/3' but apply the operation to a column family
-spec batch_put(Batch :: batch_handle(), ColumnFamily :: cf_handle(), Key :: binary(), Value :: iodata()) -> ok.
batch_put(_Batch, _ColumnFamily, _Key, _Value) ->
  ?nif_stub.

%% @doc add a merge operation to the batch
-spec batch_merge(Batch :: batch_handle(), Key :: binary(), Value :: iodata()) -> ok.
batch_merge(_Batch, _Key, _Value) ->
  ?nif_stub.

%% @doc like `batch_mege/3' but apply the operation to a column family
-spec batch_merge(Batch :: batch_handle(), ColumnFamily :: cf_handle(), Key :: binary(), Value :: iodata()) -> ok.
...
-spec transaction_put(Transaction :: transaction_handle(), Key :: binary(), Value :: iodata()) -> ok.
transaction_put(_Transaction, _Key, _Value) ->
  ?nif_stub.

%% @doc like `transaction_put/3' but apply the operation to a column family
-spec transaction_put(Transaction :: transaction_handle(), ColumnFamily :: cf_handle(), Key :: binary(),  Value :: iodata()) -> ok.
...
%% -spec transaction_merge(Transaction :: transaction_handle(), Key :: binary(), Value :: iodata()) -> ok.
%% transaction_merge(_Transaction, _Key, _Value) ->
%%   ?nif_stub.

%% %% @doc like `transaction_merge/3' but apply the operation to a column family
%% -spec transaction_merge(Transaction :: transaction_handle(), ColumnFamily :: cf_handle(), Key :: binary(), Value :: iodata()) -> ok.
